defmodule PdfTipsElixirWeb.DocumentLive.Index do
  use PdfTipsElixirWeb, :live_view

  alias PdfTipsElixir.Knowledge
  alias PdfTipsElixir.Services.{PdfProcessor, UrlProcessor, EmbeddingService}
  ...
  def handle_event("upload_pdf", _params, socket) do
    ...
    socket =
      consume_uploaded_entries(socket, :pdf, fn %{path: path}, entry ->
        uploads_dir = Path.join([:code.priv_dir(:pdf_tips_elixir), "static", "uploads"])
